defmodule EthWebSocket.WebsocketManager do
  ...
  def init(ws_url) do
    {:ok, ws_pid} = Websocket.start_link(WebSocketManager, ws_url)
    {:ok, %WebsocketManagerState{ws_pid: ws_pid, gs_pid: WebSocketManager}}
  end
  ...
  def handle_info({:perform_asynchronous_request, body}, state) do
    WebSockex.send_frame(Map.get(state, :ws_pid), {:text, body})
    {:noreply, state}
  end
    ...
    new_state =
      case decoded_res do
        %{"id" => id, "jsonrpc" => "2.0", "result" => result} ->
          GenServer.reply(Map.get(state, :from_pid), {:ok, result})
        ...
        %{"id" => _id, "jsonrpc" => "2.0", "error" => result} ->
          GenServer.reply(Map.get(state, :from_pid), {:error, result["message"]})
          state

        _ ->
          GenServer.reply(Map.get(state, :from_pid), {:error, "Unexpected server response."})
          state
      end
