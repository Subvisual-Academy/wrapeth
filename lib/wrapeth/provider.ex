defmodule Wrapeth.Provider do
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      @impl true
      def web3_client_version(pid \\ nil) do
        call_client(:web3_client_version, [], pid)
      end

      @impl true
      def web3_sha3(data, pid \\ nil) do
        call_client(:web3_sha3, [data])
      end

      @impl true
      def net_version(pid \\ nil) do
        call_client(:net_version, [], pid)
      end

      @impl true
      def net_peer_count(pid \\ nil) do
        call_client(:net_peer_count, [], pid)
      end

      @impl true
      def net_listening(pid \\ nil) do
        call_client(:net_listening, [], pid)
      end

      @impl true
      def eth_protocol_version(pid \\ nil) do
        call_client(:eth_protocol_version, [], pid)
      end

      @impl true
      def eth_syncing(pid \\ nil) do
        call_client(:eth_syncing, [], pid)
      end

      @impl true
      def eth_mining(pid \\ nil) do
        call_client(:eth_mining, [], pid)
      end

      @impl true
      def eth_hashrate(pid \\ nil) do
        call_client(:eth_hashrate, [], pid)
      end

      @impl true
      def eth_max_priority_fee_per_gas(pid \\ nil) do
        call_client(:eth_max_priority_fee_per_gas, [], pid)
      end

      @impl true
      def eth_fee_history(block_count, newestblock, reward_percentiles, pid \\ nil) do
        call_client(:eth_fee_history, [block_count, newestblock, reward_percentiles])
      end

      @impl true
      def eth_get_storage_at(address, position, block \\ "latest", pid \\ nil) do
        call_client(:eth_get_storage_at, [address, position, block])
      end

      @impl true
      def eth_get_block_transaction_count_by_hash(hash, pid \\ nil) do
        call_client(:eth_get_block_transaction_count_by_hash, [hash])
      end

      @impl true
      def eth_get_block_transaction_count_by_number(block \\ "latest", pid \\ nil) do
        call_client(:eth_get_block_transaction_count_by_number, [block])
      end

      @impl true
      def eth_get_uncle_count_by_block_hash(hash, pid \\ nil) do
        call_client(:eth_get_uncle_count_by_block_hash, [hash])
      end

      @impl true
      def eth_get_uncle_count_by_block_number(block \\ "latest", pid \\ nil) do
        call_client(:eth_get_uncle_count_by_block_number, [block])
      end

      @impl true
      def eth_get_code(address, block \\ "latest", pid \\ nil) do
        call_client(:eth_get_code, [address, block])
      end

      @impl true
      def eth_call(transaction, block \\ "latest", pid \\ nil) do
        call_client(:eth_call, [transaction, block])
      end

      @impl true
      def eth_estimate_gas(transaction, pid \\ nil) do
        call_client(:eth_estimate_gas, [transaction])
      end

      @impl true
      def eth_get_block_by_hash(hash, full, pid \\ nil) do
        call_client(:eth_get_block_by_hash, [hash, full])
      end

      @impl true
      def eth_get_transaction_by_hash(hash, pid \\ nil) do
        call_client(:eth_get_transaction_by_hash, [hash])
      end

      @impl true
      def eth_get_transaction_by_block_hash_and_index(hash, index, pid \\ nil) do
        call_client(:eth_get_transaction_by_block_hash_and_index, [hash, index])
      end

      @impl true
      def eth_get_transaction_by_block_number_and_index(block, index, pid \\ nil) do
        call_client(:eth_get_transaction_by_block_number_and_index, [block, index])
      end

      @impl true
      def eth_get_uncle_by_block_hash_and_index(hash, index, pid \\ nil) do
        call_client(:eth_get_uncle_by_block_hash_and_index, [hash, index])
      end

      @impl true
      def eth_get_compilers(pid \\ nil) do
        call_client(:eth_get_compilers, [], pid)
      end

      @impl true
      def eth_accounts(pid \\ nil) do
        call_client(:eth_accounts, [], pid)
      end

      @impl true
      def eth_get_balance(address, block \\ "latest", pid \\ nil) do
        call_client(:eth_get_balance, [address, block], pid)
      end

      @impl true
      def eth_block_number(pid \\ nil) do
        call_client(:eth_block_number, [], pid)
      end

      @impl true
      def eth_get_block_by_number(number, full, pid \\ nil) do
        call_client(:eth_get_block_by_number, [number, full])
      end

      @impl true
      def eth_get_transaction_count(address, block \\ "latest", pid \\ nil) do
        call_client(:eth_get_transaction_count, [address, block])
      end

      @impl true
      def eth_gas_price(pid \\ nil) do
        call_client(:eth_gas_price, [], pid)
      end

      @impl true
      def eth_get_transaction_receipt(hash, pid \\ nil) do
        call_client(:eth_get_transaction_receipt, [hash])
      end

      @impl true
      def eth_subscribe(envent_type \\ "newHeads", pid) do
        call_client(:eth_subscribe, [envent_type], pid)
      end

      @impl true
      def eth_unsubscribe(sub_id, pid) do
        call_client(:eth_unsubscribe, [sub_id], pid)
      end

      defp call_client(method_name, args \\ [], pid \\ nil) do
        case @client_type do
